...
> **Status in one line:** the engineering is complete and tested in paper mode (69 tests, replay demo, Docker). The broker adapters are implemented but **not yet verified against real broker servers**. **Live trading is blocked** because no strategy passes the evidence gate. Nothing is deployed continuously yet: that needs your server and API keys. See [capability status](docs/07-capability-status.md).

## Documents
1. [Broker research and recommendation](docs/01-broker-research.md): Alpaca recommended for the MVP, with trade-offs; Trading 212 is the FCA-regulated alternative
...
6. [Test results](docs/06-test-results.md) and [strategy validation results](docs/validation/RESULTS.md)
